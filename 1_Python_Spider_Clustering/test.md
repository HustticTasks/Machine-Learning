...
**根据极大似然的思想，我们可以认为，要想使预测值Y向量最逼近真实值，就是想要找到极大似然函数的估计值$$\theta$$**
...
这里$$\epsilon$$代表误差项（包括了模型误差还有噪音等） 。我们假设$$\epsilon$$符合独立同分布的 均值为零，方差是$$\sigma^2$$的高斯分布。那么$$\epsilon$$的概率密度可以表示为：

$$p(\epsilon^{(i)})=\frac{1}{\sqrt{2\pi}\sigma}exp(-\frac{(y^{(i)}-\theta^Tx^{(i)})^2}{2\sigma^2})$$

上面的概率密度意思为 在参数$$\theta$$下（这里不是限制$$\theta$$，可以当$$\theta$$不存在），给定输入$$x$$,输出预测值$$y$$的概率密度。

同理，可以把整个数据集$$X$$作为输入，整个预测向量$$Y$$作为输出，于是可以得到极大似然函数：

$$L(\theta)=L(\theta;X,\vec{y})=p(\vec{y}|X;\theta)$$

我们想要做的，就是找到一个最好的$$\theta$$，使得极大自然函数值最大，也就是，这个$$\theta$$能够预测出最准确的y值。

基于$$\epsilon$$符合独立同分布的假设，极大似然函数可以写成以下形式：
...
此时，能够使极大似然函数值最大的$$\theta$$,也必然能够使log L($$\theta$$)最大化，即：
...
​       $$=m\log\frac{1}{\sqrt{2\pi}\sigma}-\frac{1}{\sigma^2}\sum_{i=1}^m(y^{(i)}-\theta^Tx^{(i)})^2$$

于是，最大化 log L($$\theta$$)等同于 极小化

$$\frac{1}{2}\sum_{i=1}^m(y^{(i)}-\theta^Tx^{(i)})^2$$

也就是，最小二乘误差函数$$J(\theta)$$。

也就是说，最小二乘回归的解，也就是 $$\theta$$的极大似然估计，能够使得模型预测出最接近真实值的解。
...
很明显，此时采用一个特征，两个特征，无论几个特征，都不合适。因为一旦参数$$\theta$$确定，模型就确定了，那么都没办法很好地拟合。
...
- 2 由于这种只学习局部的特性，每次参数$$\theta$$都不相同。所以也需要**预测一次，就需要重新学习一次参数$$\theta$$**
...
从中可以看出，当样例$$X$$（i）越接近我们想要预测的点$$X$$，|$$X_i$$ - $$X$$|的值越小，此时权值越接近1。
